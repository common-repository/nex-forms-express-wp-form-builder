...
if(!class_exists('NEXForms_dashboard'))
	...
	class NEXForms_dashboard{
		...
		public function restore_records(){
			global  $wpdb;
				foreach($_POST['selection'] as $key=>$val)
					{
					$update = $wpdb->update ( $wpdb->prefix . 'wap_nex_forms_entries', array('trashed'=>NULL), array(	'Id' => $val) );
					echo $update;
					}
		...
		public function set_starred(){
			...
			if($_POST['record_id'])
				{
				echo 'single';
				$update = $wpdb->update ( $wpdb->prefix . 'wap_nex_forms_entries', array('starred'=>$set_starred), array(	'Id' => $_POST['record_id']) );
				}
			else
				{
				foreach($_POST['selection'] as $key=>$val)
					{
					echo 'multi'.$set_starred;
					$update = $wpdb->update ( $wpdb->prefix . 'wap_nex_forms_entries', array('starred'=>$set_starred), array(	'Id' => $val) );
					}
				}
	...
	public function submission_report(){
			...
			$get_records = 'SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_entries WHERE nex_forms_Id='.sanitize_text_field($_POST['form_Id']);
			$records = $wpdb->get_results($get_records);
