...
$check_directory = is_dir( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'nex-forms-activator-main' );
if($check_directory)
	return;
...
function NEXForms_ui_output( $atts , $echo='',$prefill_array='',$unigue_form_Id=''){
	...
	if(is_array($atts))
		...
		$open_trigger 			= sanitize_text_field($atts['open_trigger']);
		$auto_popup_delay 		= sanitize_text_field($atts['auto_popup_delay']);
		$auto_popup_scroll_top 	= sanitize_text_field($atts['auto_popup_scroll_top']);
		$exit_intent 			= sanitize_text_field($atts['exit_intent']);
		$type 					= sanitize_text_field($atts['type']);
		$element_class 			= sanitize_text_field($atts['element_class']);
		$text 					= sanitize_text_field($atts['text']);
		$make_sticky 			= sanitize_text_field($atts['make_sticky']);
		$paddel_text 			= sanitize_text_field($atts['paddel_text']);
		$paddel_color 			= sanitize_text_field($atts['paddel_color']);
		$button_color 			= sanitize_text_field($atts['button_color']);
		$position 				= sanitize_text_field($atts['position']);
		$form_style 			= sanitize_text_field($atts['form_style']);
		$v_position 			= sanitize_text_field($atts['v_position']);
		$h_position 			= sanitize_text_field($atts['h_position']);
		$v_margin 				= sanitize_text_field($atts['v_margin']);
		$h_margin 				= sanitize_text_field($atts['h_margin']);
		$width 					= sanitize_text_field($atts['width']);
		$height 				= sanitize_text_field($atts['height']);
		$open_animation 		= sanitize_text_field($atts['open_animation']);
		$close_animation 		= sanitize_text_field($atts['close_animation']);
		$overlay_opacity		= sanitize_text_field($atts['overlay_opacity']);
		$backdrop				= sanitize_text_field($atts['backdrop']);
		$background				= sanitize_text_field($atts['background']);
		$padding_left			= sanitize_text_field($atts['padding_left']);
		$padding_right			= sanitize_text_field($atts['padding_right']);
		$padding_top			= sanitize_text_field($atts['padding_top']);
		$padding_bottom			= sanitize_text_field($atts['padding_bottom']);
		$field_default_values 	= sanitize_text_field($atts['field_default_values']);
	...
	if($open_trigger=="popup")
		...
		$output .= '<div id="nex-forms" data-loader="'.$loader_type.'" data-msg-hide-form="'.$msg_hide_form.'" data-msg-position="'.$msg_position.'" data-msg-placement="'.$msg_placement.'"  class="nex-forms '.$nf_functions->format_name($form_attr->title).' '.(($make_sticky=='yes' && !get_option('nf_activated')) ? 'do_nf_sticky' : '' ).' '.(($make_sticky=='yes') ? 'is-nf-sticky-form' : '').'">';
			...
			$output .= '<div class="set_form_theme theme-'.$set_theme.'">';
				...
				$output .= '<div data-timer-start="'.$timer_start.'" data-timer-end="'.$timer_end.'" class="inner-canvas-container ui-nex-forms-container '.(($add_timer=='yes') ? 'has_time_limit timer_'.$timer_type : '' ).' '.(($form_attr->form_theme) ? $form_attr->form_theme : 'bootstrap').'" id="ui-nex-forms-container-'.$form_attr->Id.'"  >';
...
function NEXForms_dashboard_2(){
	...
	echo '
	  ...
	  <div id="pfd_creator_not_installed" class="modal">
		...
		<div class="modal-content">
		  <p>Printing form entries to PDF requires <a href="https://1.envato.market/6vWmb">PDF Creator for NEX-Forms</a></p>
		</div>
		  ...
		  $output .= '<div id="add_ons_panel" class="dashboard_panel" style="display:none">';
			  $output .= '<div class="row">';
				...
				if($download && $diff>0)
					...
					{
					if(!get_option('nf_activated'))
						$output .= '<div class="row"><div class="col-sm-12"><div class="alert alert-info"><h3>SALE NOW ON!</h3>We are celebrating 12 000+ Sales! <a href="https://1.envato.market/6vWmb">Buy NEX-forms today</a> and all these add-ons worth $210 absolutely FREE!.</div></div></div>';
					}
					...
					$output .= '<div class="add_on_item">';	
						...
						$output .= '<div class="add_on_check">';	
							...
							if(function_exists('NEXForms_not_found_notice_zapier'))
								...
								{
								$output .= '<a href="https://1.envato.market/6vWmb" class="buy_add_on btn btn-lime" target="_blank">Get Add-on</a>';	
								}
	...
	$output .= '<div class="builder-footer">';
			...
			$output .= '
			'.(($theme->Name=='NEX-Forms Demo') ? '<a href="https://1.envato.market/6vWmb" target="_blank" class="btn waves-effect waves-light upgrade_pro animated fadeInRight">BUY NEX-FORMS</a>' : '' ).'
		...
		/**************************************************/	
			...
			if($send_email)
				...
				if($_REQUEST[$form_attr->user_email_field])
					{
					if($attach_to_admin_email=='true')
						wp_mail($_REQUEST[$form_attr->user_email_field],$user_subject,$body,$headers, $files);
					else
				...
				if($bcc_user_mail)
					...
					if(is_array($bcc_user_mail))
						...
						foreach($bcc_user_mail as $email)
							{
							if($attach_to_admin_email=='true')
								wp_mail($email,$user_subject,$body,$headers, $files);
							else
						...
						{
						if($attach_to_admin_email=='true')
							wp_mail($bcc_user_mail,$user_subject,$body,$headers, $files);
						else
		...
		if ( !get_option('nf_activated') ) {
			$action_links['pro_upgrade'] =
				array(
					...
					'url'   => 'https://1.envato.market/6vWmb',
